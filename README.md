...
简单来说，它是一个代理软件，可以用来科学上网（翻墙）学习国外先进科学技术。
...
由于许多 V2Ray 用户都有使用过 Shadowsocks 的经验，基本上可以按照使用 Shadowsocks 那样使用。但是 V2Ray 还是和 Shadowsocks 不太一样，所以我大概说一下使用上的差异。请注意，差异不代表好坏或优劣，如果一个事物有某种东西就要求其他同类的事物也要拥有，那么它也就没有了存在的意义。
...
- 路由器翻墙：实际上他们并没有什么区别，不要以为没有插件就不能在路由器上用，看事物请看本质。

### 白话文教程有很多问题

首先我从未标榜过我写的东西一定正确、没有问题；其次本文所有内容都是放在 Github 上开源的，所有人都可以贡献、修正。哪怕有人发现了有错漏的地方在群里说一声，我看到了也会修改。像这种干巴巴地说xx有问题但又指不出具体是什么问题的我只能认为问题在于那个人自身。

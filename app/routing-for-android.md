...
开启的方法是在 inbound 和 inboundDetour 中加入 `"domainOverride": ["http", "tls"]`，如：
```javascript
{
  "routing": {},
  "inbound": {
...
不过有一个方法可以曲线救国：
 使用 [V2RayGo 的配置](https://github.com/xiaokangwang/AndroidLibV2ray/blob/master/conf/conf_vpnservice.json)，将里面所有的 inbound 和 inboundDetour 加入 `"domainOverride": ["http", "tls"]`，再将 outbound 和 routing 修改成你实际需要的配置，其它的不要动。修改完之后再导入 Actinium 中，Actinium 不会改你的文件了。
